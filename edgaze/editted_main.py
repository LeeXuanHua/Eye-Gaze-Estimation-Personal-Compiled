...
def main():
    ...
    logging.basicConfig(filename=f"{result_prefix}/{args.model}.log", level=logging.INFO, filemode="a", format='[%(asctime)s]:%(levelname)5s:%(message)s')
    ...
    for i in range(seq_start, seq_end):
        ...
        start = initial
        logging.info("Loading pre-trained network for baseline")
        ...
        logging.info(f"Loaded pre-trained network for baseline in {end-start}")
        start = end

        logging.info("Loading smart camera filter instance")
        ...
        logging.info(f"Loaded smart camera filter instance in {end-start}")
        start = end

        logging.info("Initializing deepvog")
        ...
        logging.info(f"Initialized deepvog in {end-start}")
        start = end

        # Required for 
        # This prevents FileNotFoundError - E.g. FileNotFoundError: [Errno 2] No such file or directory: 'eye_fit_models/S_0.json'
        # logging.info(f"Initialising to fit eyeball model")
        # # Fit an eyeball model.
        # inferer.process(dataset, mode="Fit", keyword=keyword)

        # # store the eye model
        # inferer.save_eyeball_model(model_name)
        # logging.info(f"Eyeball model stored")

        
        logging.info("Loading eyeball model")
        # load the eyeball model
        inferer.load_eyeball_model(model_name)
        end = time.perf_counter()
        logging.info(f"Loaded eyeball model in {end-start}")
        start = end

        logging.info("Inferring gaze")
        # infer gaze
        inferer.process(
            dataset,
            mode="Infer",
        ...
        logging.info(f"Inferred gaze in {end-start}")

        logging.info(f"Finished processing sequence in {end - initial}")
        logging.info("=========================================")
