...
logging.basicConfig(filename=f"./demo/demo.log", level=logging.INFO, filemode="a", format='[%(asctime)s]:%(levelname)5s:%(message)s')
...
if num_modes != 1:
    ...
    if sum([x is not None for x in cli_modes_list]) > 0:
        ...
        if args.fit is not None:
            ...
            logging.info(f"Fitted in {end-start}")
            start = end
        ...
        if args.infer is not None:
            ...
            logging.info(f"Inferred in {end-start}")
            start = end

        logging.info(f"Finished processing in {end - initial}")
        logging.info("=========================================")